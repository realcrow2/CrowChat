window.APP = {
	...
	mounted() {
		post('http://CrowChat/loaded', JSON.stringify({}));
	...
	methods: {
		...
		ON_CLEAR() {
			...
			if (this.oldMessages && this.oldMessages.length > 0) {
		...
		send(e) {
			if(this.message !== '') {
				post('http://CrowChat/chatResult', JSON.stringify({
		...
		hideInput(canceled = false) {
			if (canceled) {
				post('http://CrowChat/chatResult', JSON.stringify({ canceled }));
			}
			this.message = '';
